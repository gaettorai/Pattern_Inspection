...
IMPLEMENT_DYNAMIC(DIALOG_MANUAL_UPPER, CDialogEx)

const int m_nLineSize = 15;
const int m_nBasic = 95;
const int m_nRepeat = 10;

DIALOG_MANUAL_UPPER::DIALOG_MANUAL_UPPER(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DIALOG_MANUAL_UPPER, pParent)
{

}

DIALOG_MANUAL_UPPER::~DIALOG_MANUAL_UPPER()
{
}

void DIALOG_MANUAL_UPPER::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_PIC_MANUAL_UPPER, m_pic_manual_upper);
}


BEGIN_MESSAGE_MAP(DIALOG_MANUAL_UPPER, CDialogEx)
	ON_WM_SIZE()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	ON_WM_CREATE()
...
// DIALOG_MANUAL_UPPER 메시지 처리기


BOOL DIALOG_MANUAL_UPPER::OnInitDialog()
{
	CDialogEx::OnInitDialog();

	// TODO:  여기에 추가 초기화 작업을 추가합니다.

	GetDlgItem(IDC_PIC_MANUAL_UPPER)->MoveWindow(0, 0, 512, 512);
...
BOOL DIALOG_MANUAL_UPPER::PreTranslateMessage(MSG* pMsg)
{
...
void DIALOG_MANUAL_UPPER::OnSize(UINT nType, int cx, int cy)
{
...
void DIALOG_MANUAL_UPPER::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void DIALOG_MANUAL_UPPER::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	int delta;
...
int DIALOG_MANUAL_UPPER::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void DIALOG_MANUAL_UPPER::click_magni()
{
	RECT rc;
	GetClientRect(&rc);
...
void DIALOG_MANUAL_UPPER::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
...
void DIALOG_MANUAL_UPPER::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	CREVIEWAREADlg *pDlg = (CREVIEWAREADlg*)AfxGetMainWnd();
	...
	if (pDlg->dlg_manual->measure_mode != 0)
	{
		...
		if (pDlg->dlg_manual->measure_mode == 1)
		{
			...
			{
				CRect rt;
				CStatic * pStatic = (CStatic*)GetDlgItem(IDC_PIC_MANUAL_UPPER);
		...
		// RECTANGLE
		else if (pDlg->dlg_manual->measure_mode == 2 || pDlg->dlg_manual->measure_mode == 4 || pDlg->dlg_manual->measure_mode == 5 || pDlg->dlg_manual->measure_mode == 6)
		{
			if (click_count == 1)
			{
				CRect rt;
...
void DIALOG_MANUAL_UPPER::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	CREVIEWAREADlg *pDlg = (CREVIEWAREADlg*)AfxGetMainWnd();
	...
	if (pDlg->dlg_manual->measure_mode != 0)
	{
		...
		if (pDlg->dlg_manual->measure_mode == 1)
		{
			...
			if (pDlg->dlg_manual->m_cmb_magni.GetCurSel() == 0)
				pDlg->dlg_auto->DrawLine(first_point, second_point, 1, str_line);
		...
		else if (pDlg->dlg_manual->measure_mode == 2 || pDlg->dlg_manual->measure_mode == 4 || pDlg->dlg_manual->measure_mode == 5 ||  pDlg->dlg_manual->measure_mode == 6)
		{
			if (click_count == 1)
			{
				...
				if (pDlg->dlg_manual->measure_mode == 2)
				{
					...
					if (pDlg->dlg_manual->m_cmb_magni.GetCurSel() == 0)
						pDlg->dlg_auto->DrawRectangle(first_point, second_point, 1, str_ver, str_hor);
				...
				else if (pDlg->dlg_manual->measure_mode == 6)
				{
					pDC->SelectObject(pen_rectangle);
					pDC->SelectObject(&null_brush);
					...
					StretchDIBits(pDlg->dlg_manual->hdc_mark_upper, 0, 0, 100, 100, 0, 0, manual_upper_mark.cols, manual_upper_mark.rows, manual_upper_mark.data, markInfo, DIB_RGB_COLORS, SRCCOPY);

					pDlg->dlg_manual->GetDlgItem(IDC_BTN_UPPER_MARK_SAVE)->EnableWindow(TRUE);
				}
			}
		}
		...
		else if (pDlg->dlg_manual->measure_mode == 3)
		{
			...
			if (pDlg->dlg_manual->m_cmb_magni.GetCurSel() == 0)
			{
				pDlg->dlg_auto->DrawCircle(first_point, second_point, radius, 1, str_circle);
			}
