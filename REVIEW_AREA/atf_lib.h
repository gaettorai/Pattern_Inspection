 ...
 *			06/03/2017 Gabriel G.       Added atf_getObjectiveNumOffsetz()
 *
 *****************************************************************************/
...
#if _MSC_VER < 1900 //MSVC version < 2015
...
//NS: called atf_PingAck() to see if communication with sensor is established or not
int atf_OpenConnection (char *port, int speed);
int atf_OpenConnectionByNamedPipe ( char *path, int bufferSize, int refreshDelay );
...
int atf_sendCommand( const unsigned char* const pLine, DWORD cbLineBufferLength, unsigned char *pResponse, DWORD cbResponseBufferLength, DWORD &cbResponseLength );
...
int atf_Make0();
//set the focus ref point
int atf_SetRef();
int atf_RemoveFocusRef();

//XXX: AS-77 ----------------------------------------------------------------
int atf_ReadMake0Offsets( int iObj, int iSurface, int iAbsIdxZero[ 24 ] );
int atf_WriteMake0Offsets( int iObj, int iSurface, int iAbsIdxZero[ 24 ] );
//---------------------------------------------------------------------------
...
int atf_ReadWindowIntegrationTimes(int obj, WindowEnm which, short* pit);
int atf_WriteWindowIntegrationTimes(int obj, WindowEnm which, short it);

// reads/writes line width parameter
int atf_ReadLineWindowWidth(int obj, WindowEnm which, short* pwidth);
int atf_WriteLineWindowWidth(int obj, WindowEnm which, short width);

// reads/writes laser power limit
int atf_WriteLaserPowerLimit(int iObj, short lplimit);
int atf_ReadLaserPowerLimit(int iObj, short *plplimit);

// read/write snr threshold
int atf_ReadWindowSnr(int obj, WindowEnm which, float* pdsnr);
int atf_WriteWindowSnr(int obj, WindowEnm which, float snr);
...
//if adaptive_lim=2, set to the abs limit in um option
...
// NS(AL):-Read sensor position, status and error code through one function
int atf_ReadErrorCodeStatusPosition(AtfCodesEnm *pcode, StatusFlagsEnm *pStatus, float* pfpos);
...
// h_len: number of elements of h_scanline, user has to provide a value for this,70 for each segment, for 7 segment 7x70=490, plus some margin 600
// v_len: number of elements of v_scanline, user has to provide a value for this,300 for each segment. If using 3 segment provide 3x300=900, for 7-segment : 300x7=2100 plus some margin 2200
...
int atf_WriteLaserPower(u_short laser, int iByteNumToWrite = 1, int dotIdx = 0);
...
void atf_getObjectiveNumOffsetz( float fObjPitch[ MAX_LLC_OBJ_NUM ] );
...
// checks if communication with EOA hardware is available
bool atf_ooa_isEoaOK(void);
...
bool atf_ooa_MoveUStep(int iMoveUstep, bool bWait);
...
//  executes relative move of specified number of micrometers that are calibrated to the current objective. T
// This feature is only supported on EOA
bool atf_ooa_MoveUmDepth(float fMoveUm, bool bWait);
// This feature is only supported on EOA. It provides current hw position, temperature (Deg C) and status
// status==3 is operational (see EooStatus definitions), other values indicate device is not perfroming 
// up to the expectation 
bool atf_ooa_GetEoaHwStatus(int *pHwPwm, float *pfHwTempCDeg, int *pEoaHwStatus);

// OOA end-------------------------------------------------------------------------------------

// sets window mode to the windowMode value. 
// WindowNear1Enm - sets obj objective near mode to operates in a single line mode.  
// WindowNear7Enm - sets obj objective near mode to operates in 7 segment mode. 
// neighter choice affects auto window transit
int atf_NearWindowModeSet(int obj, WindowEnm windowMode);
// returns current windowMode value. 
int atf_NearWindowModeGet(int obj, WindowEnm *pWindowMode);

// sets Near-Far auto window change flag so called auto transit
int atf_WindowAutoTransitSet(bool bIsAutoTransitEnabled);
// gets Near-Far auto window change flag
int atf_WindowAutoTransitGet(bool *pbIsAutoTransitEnabled);

// WindowNear1Enm - 
// WindowNear7Enm - objective is forced to operate in a near mode, 1 or 7 segment depending on atf_NearWindowModeSet()
// WindowAutoEnm - objective is allowed to make far-near transition if atf_WindowAutoTransitSet is set
// WindowFarEnm - objective is forced to work in far mode. 
int atf_WindowCurrentModeSet(WindowEnm windowMode);
// reads current mode as above
int atf_WindowCurrentModeGet(WindowEnm *pWindowMode);
...
//For Atf6 sensors auto find slope, and determine and set the best zero for sensor
//default Range=0, the sensor will auto determin the search range 
//if the bFindSlope=true, the function will find and set the slope.
//alway leave the flag=0
int atf_SysTune(int iRange=0,  bool bFindSlope=true, int flag=0);


//********************************************************************************************************************************///
/*********************************************Functions related to Dover Stage***************************************************///
bool atf_ReadOptionStrap(USHORT * option);
bool atf_DOF_Home( int *piErrCode);
//bool atf_DOF_IsInHomePosition(bool *pbInHome);
bool atf_DOF_GetUpperLimit(float *upperLimitm, int *piErrCode);
bool atf_DOF_GetLowerLimit(float *lowerLimitmm, int *piErrCode);
bool atf_DOF_GetStatus(USHORT *pDofStatus);
bool atf_DOF_GetEncoderPosition(float *presentpos);
bool  atf_DOF_RecoverStage(int *piErrCode);
//int atf_sendDOF(char *toSend, char*toRecv);



/*********************************************************************************************************************************
pfa functions, only work with pfa sensors
*/
bool pfa_IsPfaSensor();
int pfa_SetOperationMode(unsigned int mode);
int pfa_ReadOperationMode(unsigned int *mode);

//pMagList must be allocated to hold up to 8 short int
int pfa_ReadDefMagList(short *pMaglist);

int pfa_OpenTcpIpConnection(const char *pIpAddress, int iPortNum);
void pfa_CloseTcpIpConnection();
bool pfa_isTcpIpConnection();

//firt 4 char are ip, then next 4 are subnet, uint8_t allocate ip_subnet[8]
int pfa_read_ip_subnet(uint8_t *ip_subnet);
int pfa_write_ip_subnet(uint8_t *ip_subnet);
#ifdef __cplusplus
	}
#endif
